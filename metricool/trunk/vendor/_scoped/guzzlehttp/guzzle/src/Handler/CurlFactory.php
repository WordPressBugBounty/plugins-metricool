...
namespace Metricool\Vendor\GuzzleHttp\Handler;

use Metricool\Vendor\GuzzleHttp\Exception\ConnectException;
use Metricool\Vendor\GuzzleHttp\Exception\RequestException;
use Metricool\Vendor\GuzzleHttp\Multiplexing;
use Metricool\Vendor\GuzzleHttp\Promise as P;
use Metricool\Vendor\GuzzleHttp\Promise\FulfilledPromise;
use Metricool\Vendor\GuzzleHttp\Promise\PromiseInterface;
use Metricool\Vendor\GuzzleHttp\Psr7;
use Metricool\Vendor\GuzzleHttp\Psr7\LazyOpenStream;
use Metricool\Vendor\GuzzleHttp\Psr7\Uri;
use Metricool\Vendor\GuzzleHttp\TransferStats;
use Metricool\Vendor\GuzzleHttp\TransportSharing;
use Metricool\Vendor\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function triggerConflictingCurlOptionDeprecations(array $options): void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            if ($replacement !== null) {
                \Metricool\Vendor\trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed request handling. Use %s instead.', $name, $replacement));
                continue;
            }
            \Metricool\Vendor\trigger_deprecation('guzzlehttp/guzzle', $since, \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed cURL internals.', $name));
        }
    }
    private static function triggerUnsupportedCurlOptionDeprecations(array $options): void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            \Metricool\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject raw cURL options outside the built-in cURL handlers\' allow-list.', self::formatCurlOption($option)));
        }
    }
    private static function triggerUnsupportedRequestOptionDeprecations(array $options): void
    {
        if (\array_key_exists('stream_context', $options)) {
            \Metricool\Vendor\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "stream_context" request option to a cURL handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because cURL handlers ignore PHP stream context options.');
        }
    }
    /**
